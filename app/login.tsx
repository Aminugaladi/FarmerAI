...
export default function LoginScreen() {
    ...
    const handleForgotPassword = async () => {
        ...
        Alert.alert(
            ...
            `Shin kana son mu tura maka link din canza password zuwa ${email}?`,
            [
                { text: 'A fasa', style: 'cancel' },
                {
                    text: 'Eh, tura',
                    onPress: async () => {
                        try {
                            await sendPasswordResetEmail(auth, email);
                            Alert.alert('An Tura!', 'Duba akwatin sakon imel dinka don sake saita kalmar sirri.');
                        } catch (error: any) {
                            Alert.alert('Kuskure', 'Ba a samu wannan imel din ba ko kuma akwai matsalar yanar gizo.');
                        }
                    }
                }
            ]
...
const styles = StyleSheet.create({
    ...
    headerArea: { alignItems: 'center', marginBottom: 40, marginTop: 50 },
    logoText: { fontSize: 28, fontWeight: 'bold', color: '#2E7D32', textAlign: 'center' },
    subTitle: { fontSize: 16, color: '#666', textAlign: 'center', marginTop: 10 },
    inputArea: { backgroundColor: '#fff', padding: 25, borderRadius: 20, elevation: 4 },
    label: { fontSize: 14, fontWeight: '600', color: '#333', marginBottom: 8, marginTop: 15 },
    input: { borderWidth: 1, borderColor: '#E0E0E0', borderRadius: 12, padding: 15, fontSize: 16, backgroundColor: '#FAFAFA' },
    loginButton: { backgroundColor: '#2E7D32', padding: 16, borderRadius: 12, alignItems: 'center', marginTop: 30 },
    buttonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
    registerLink: { marginTop: 25, alignItems: 'center' },
    registerLinkText: { color: '#2E7D32', fontSize: 15 },
    forgotPass: { marginTop: 15, alignItems: 'center' },
    forgotPassText: { color: '#888', fontSize: 14, textDecorationLine: 'underline' },
});